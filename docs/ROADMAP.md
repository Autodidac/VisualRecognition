...
## Current State
- Win32 UI (`src/interface_app.ixx`) captures a patch around the cursor and displays it in a preview area.
...
- The `PatchRecognizer` (`src/vision_recognition_engine.ixx`) stores labeled patches, performs cosine-similarity matching, and saves/loads models to `pixelai_examples.bin`.
...
3. **Stability**: guard Win32 handles in `interface_app.ixx` (preview/status HWNDs, hooks) to reduce leak risk on failures.
4. **Testing Hooks**: add minimal unit coverage for `PatchRecognizer` (cosine similarity, persistence round-trip) using a cross-platform test target.
...
- [ ] Extract preview drawing into a smaller helper to simplify `interface_layout.ixx` and enable potential resizing modes.
