    ...
    public:
            ...
            float* out_score = nullptr) const
        {
            ...
            const auto downsample =
                [](span<const std::uint32_t> src,
                    int w, int h,
                    int targetW,
                    int targetH) -> vector<float>
                {
                    vector<float> out;
                    out.resize(static_cast<std::size_t>(targetW * targetH) * 3u);
                    ...
                    for (int ty = 0; ty < targetH; ++ty)
                    {
                        ...
                        for (int tx = 0; tx < targetW; ++tx)
                        {
                            ...
                            const std::uint32_t px =
                                src[static_cast<std::size_t>(syi * w + sxi)];
            ...
            const vector<float> queryFeat =
                downsample(pixels, width, height, kFeatW, kFeatH);

            auto distance_sq =
                [](span<const float> a, span<const float> b) -> float
                {
            ...
            {
                span<const std::uint32_t> exSpan{
                    ex.pixels.data(),
                    ex.pixels.size()
                };

                tmp = downsample(exSpan, ex.width, ex.height, kFeatW, kFeatH);

                const float d = distance_sq(
                    span<const float>(queryFeat.data(), queryFeat.size()),
                    span<const float>(tmp.data(), tmp.size())
                );
        ...
        [[nodiscard]] bool save_to_file(const string& path) const
        {
            ...
            const std::uint32_t count =
                static_cast<std::uint32_t>(examples_.size());
        ...
        [[nodiscard]] bool load_from_file(const string& path)
        {
            ...
            constexpr std::size_t   kMaxPixels =
                static_cast<std::size_t>(kMaxDim) * kMaxDim;
            ...
            for (std::uint32_t i = 0; i < count; ++i)
            {
                ...
                const std::size_t expected_pixels =
                    static_cast<std::size_t>(expected_pixels64);
